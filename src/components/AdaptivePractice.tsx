...
export const AdaptivePractice: React.FC<AdaptivePracticeProps> = ({
  ...
  const handleSubmit = () => {
    if (!currentExercise || !selectedAnswer || !justification.trim()) {
      alert('Please pick an answer and write down how you got it!');
      return;
    }
    ...
    <div className="max-w-4xl mx-auto p-6 bg-white rounded-lg shadow-lg">
      <div className="mb-6">
        <div className="flex justify-between items-center mb-2">
          <h2 className="text-2xl font-bold text-gray-800">Practice Problem {currentIndex + 1} of {PRACTICE_EXERCISE_COUNT}</h2>
          <div className="text-sm text-gray-600">
            You've gotten {currentAccuracy.toFixed(0)}% correct so far!
          </div>
        </div>
      ...
      <div className="mb-6">
        ...
        <textarea
          ...
          disabled={showFeedback}
          placeholder="Example: I compared both fractions to ½. Since 1/3 is less than ½ and 2/3 is greater than ½, I know 1/3 is less than 2/3."
          ...
          <div className={`p-4 rounded-lg mb-4 ${
            ...
            {!isCorrect && (
              <p className="text-sm text-gray-700 mt-2">
                <strong>Hint:</strong> Try using {currentExercise.method === 'benchmark' ? 'benchmarks (compare to 0, ½, or 1)' : currentExercise.method === 'common-denominator' ? 'making the bottom numbers the same' : 'multiplying across diagonally'} to compare these fractions.
              </p>
            )}
          </div>
          <button
